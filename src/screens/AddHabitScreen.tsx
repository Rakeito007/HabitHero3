...
import { View, Text, TextInput, ScrollView, Pressable, Keyboard, Alert } from 'react-native';
...
const AddHabitScreen: React.FC<AddHabitScreenProps> = ({ navigation }) => {
  const { addHabit, habits, settings } = useHabitStore();
  const isFreePlan = settings.subscriptionStatus === 'free';
  const habitCount = habits.filter(h => !h.archived).length;
  const theme = getTheme(settings.theme);
  
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [selectedColor, setSelectedColor] = useState(settings.defaultHabitColor);
  const [selectedIcon, setSelectedIcon] = useState(settings.defaultHabitIcon);
  ...
  const handleSave = () => {
    ...
    try {
      ...
      showSuccessNotification(
        'Habit Created!',
        `${name.trim()} has been added to your habits.`
      );
          ...
          {isFreePlan && habitCount >= 3 && (
            ...
            >
              <View className="flex-row items-center">
                ...
                >
                  Free plan limited to 3 habits. Upgrade to Pro for unlimited habits.
                </Text>
              </View>
            </View>
          )}
